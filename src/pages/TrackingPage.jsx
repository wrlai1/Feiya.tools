...
// ─── SKU view — one card per SKU with size tiles + drillable entries ──────────
function SKUGroup({ sku, rows, onSelectTracking }) {
  ...
  const bySizeMap = useMemo(() => {
    ...
    rows.forEach((r) => {
      ...
      if (!map[size]) map[size] = { size, totalQty: 0 }
  ...
  return (
    <div className="card overflow-hidden">
      ...
      {/* Tracking entries toggle */}
      <div className="px-4 pb-3">
        <button
          onClick={() => setShowEntries((v) => !v)}
          className="flex items-center gap-1.5 text-xs font-medium text-slate-500 hover:text-blue-600 transition-colors mt-1"
        >
          {showEntries
            ? <ChevronDown className="w-3.5 h-3.5" />
            : <ChevronRight className="w-3.5 h-3.5" />}
          {showEntries ? 'Hide' : 'View'} {rows.length} tracking entr{rows.length !== 1 ? 'ies' : 'y'}
        </button>

        {showEntries && (
          <div className="mt-2 rounded-xl border border-slate-200 overflow-hidden">
            {/* Column headers */}
            <div className="grid grid-cols-[1fr_100px_60px_36px] gap-2 px-3 py-2 bg-slate-50 border-b border-slate-200">
              <span className="text-xs font-semibold text-slate-400 uppercase tracking-wide">Tracking #</span>
              <span className="text-xs font-semibold text-slate-400 uppercase tracking-wide">True Size</span>
              <span className="text-xs font-semibold text-slate-400 uppercase tracking-wide">Qty</span>
              <span />
            </div>
            ...
            <div className="divide-y divide-slate-100">
              {rows.map((row, idx) => (
                <div
                  key={idx}
                  className="grid grid-cols-[1fr_100px_60px_36px] gap-2 px-3 py-2.5 items-center hover:bg-slate-50 transition-colors"
                >
                  {/* Tracking number */}
                  <span className="font-mono text-xs text-slate-700 truncate">{row.tracking || '—'}</span>

                  {/* True size badge */}
                  <div>
                    {row.actualSize
                      ? <span className="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-semibold bg-blue-100 text-blue-700">{row.actualSize}</span>
                      : <span className="text-xs text-slate-400">—</span>}
                  </div>
                  ...
                  <div>
                    <span className={`inline-flex items-center justify-center w-8 h-6 rounded-md text-xs font-bold ${
                      (row.quantity || 0) >= 10 ? 'bg-green-100 text-green-700'
                      : (row.quantity || 0) >= 1  ? 'bg-amber-100 text-amber-700'
                      : 'bg-red-100 text-red-600'
                    }`}>
                      {row.quantity ?? 0}
                    </span>
                  ...
                  {/* Jump to tracking # button */}
                  <button
                    onClick={() => onSelectTracking(row.tracking)}
                    className="w-7 h-7 flex items-center justify-center rounded-lg text-slate-400 hover:text-white hover:bg-teal-500 transition-all"
                    title={`View tracking ${row.tracking}`}
                  >
