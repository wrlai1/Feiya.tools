...
import inventoryEmail from './inventoryEmail.cjs'

const { authenticateUser } = authentication
const { ATTACHMENT_ONLY_HTML, buildInventoryEmail, buildInventoryWorkbook, localDateParts, normalizeSettings, previousDate, shouldSendDailyReport } = inventoryEmail
...
async function sendReport(sql, settings, reportDate, force = false) {
  ...
  try {
    ...
    const [latestMovement] = await sql`
      SELECT MAX(COALESCE(rows.business_day, rows.applied_at::date))::text AS movement_day
      FROM inventory_txn_rows rows
      WHERE rows.txn_type = 'sales'
        AND UPPER(rows.style) = ANY(${settings.styles}::text[])
        AND COALESCE(rows.business_day, rows.applied_at::date) <= ${reportDate}::date
        AND NOT EXISTS (
          SELECT 1 FROM inventory_transactions transactions
          WHERE transactions.id = rows.transaction_id
            AND transactions.rolled_back_at IS NOT NULL
        )
    `
    const movementDay = latestMovement?.movement_day || previousDate(reportDate)
    const movements = await sql`SELECT rows.style, rows.color, rows.size,
      COALESCE(SUM(rows.qty) FILTER (
        WHERE rows.txn_type = 'sales'
          AND COALESCE(rows.business_day, rows.applied_at::date) = ${movementDay}::date
      ), 0)::int AS sales,
      COALESCE(SUM(rows.qty) FILTER (
        WHERE rows.txn_type = 'return'
          AND COALESCE(rows.business_day, rows.applied_at::date) = ${movementDay}::date
      ), 0)::int AS returns,
      COALESCE(SUM(rows.qty) FILTER (
        WHERE rows.txn_type = 'sales'
          AND COALESCE(rows.business_day, rows.applied_at::date) >= ${movementDay}::date - 6
      ), 0)::int AS sales_7,
      COALESCE(SUM(rows.qty) FILTER (
        WHERE rows.txn_type = 'sales'
          AND COALESCE(rows.business_day, rows.applied_at::date) >= ${movementDay}::date - 13
      ), 0)::int AS sales_14,
      COALESCE(SUM(rows.qty) FILTER (WHERE rows.txn_type = 'sales'), 0)::int AS sales_30,
      COALESCE(SUM(rows.qty) FILTER (WHERE rows.txn_type = 'return'), 0)::int AS returns_30
      FROM inventory_txn_rows rows
      WHERE UPPER(rows.style) = ANY(${settings.styles}::text[])
        AND COALESCE(rows.business_day, rows.applied_at::date) >= ${movementDay}::date - 29
        AND COALESCE(rows.business_day, rows.applied_at::date) <= ${movementDay}::date
    ...
    const report = buildInventoryEmail({ reportDate, movementDay, inventory, movements })
    const workbook = await buildInventoryWorkbook({ ...report, reportDate, movementDay })
    const response = await fetch('https://api.resend.com/emails', {
      method: 'POST',
      headers: { Authorization: `Bearer ${process.env.RESEND_API_KEY}`, 'Content-Type': 'application/json' },
      body: JSON.stringify({
        from: process.env.INVENTORY_EMAIL_FROM,
        to: settings.recipients,
        subject: report.subject,
        html: ATTACHMENT_ONLY_HTML,
        attachments: [{
          filename: `daily_sales_inventory_${movementDay}.xlsx`,
...
export default async function handler(req, res) {
  try {
    ...
    if (action === 'email-cron') {
      ...
      if (!shouldSendDailyReport(local.hour, settings.sendHour)) return res.json({ skipped: true, reason: 'Before configured send hour' })
    ...
    console.error('[/api/inventory-balance?email]', error)
