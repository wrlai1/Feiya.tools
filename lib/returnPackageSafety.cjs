...
const FINAL_RETURN_STATUSES = new Set(['received', 'discrepancy', 'rejected', 'cancelled'])

function isFinalReturnStatus(status) {
  return FINAL_RETURN_STATUSES.has(String(status || '').trim().toLowerCase())
}

function returnWorkflowActions({
  status,
  role,
  requiresItemResolution = false,
  hasItems = false,
} = {}) {
  const normalizedStatus = String(status || '').trim().toLowerCase()
  const admin = role === 'admin'
  if (isFinalReturnStatus(normalizedStatus) || normalizedStatus === 'processing') return []
  if (!['pending', 'needs_review'].includes(normalizedStatus)) return []
  const actions = new Set()
  if (admin) {
    actions.add('close_cancelled')
    actions.add('reassign_store')
    actions.add('close_not_ours')
    if (hasItems) actions.add('remap_sku')
    if (requiresItemResolution) {
      actions.add('resolve_replace')
      actions.add('resolve_append')
      if (hasItems) actions.add('resolve_keep_existing')
    } else {
      actions.add('confirm')
    }
  } else {
    actions.add('send_admin_review')
    if (normalizedStatus === 'pending' && !requiresItemResolution && hasItems) {
      actions.add('confirm_all_good')
    }
  }
  return [...actions]
}
...
function normalizeReturnItemResolutionMode(value) {
  const mode = String(value || '').trim().toLowerCase()
  if (!['keep_existing', 'replace', 'append'].includes(mode)) {
    throw new Error('Choose keep_existing, replace, or append before resolving return items')
  }
  return mode
}

function resolveReturnPackageItems(existingItems, resolvedItems, rawMode) {
  const mode = normalizeReturnItemResolutionMode(rawMode)
  if (mode === 'keep_existing') {
    const items = mergeReturnPackageItems(existingItems, [])
    if (!items.length) throw new Error('This package has no existing items to keep')
    if ((resolvedItems || []).length) {
      throw new Error('keep_existing cannot include new return items')
    }
    return items
  }
  if (!(resolvedItems || []).length) {
    throw new Error(`${mode} requires at least one resolved return item`)
  }
  return mode === 'replace'
    ? mergeReturnPackageItems([], resolvedItems)
    : mergeReturnPackageItems(existingItems, resolvedItems)
}
...
function normalizeManualReturnDraft({
  trackingNumber,
  storeName,
  storeKey,
  username = '',
  ...
  return {
    ...
    status: isAdmin ? 'needs_review' : 'pending',
    expected_units: 0,
