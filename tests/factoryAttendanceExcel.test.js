...
test('attendance workbook combines multiple days in one simple sheet', () => {
  const days = [
    {
      ...
      punchCount: 4, workedMinutes: 590, needsReview: false,
    ...
    },
    {
      ...
      punchCount: 3, workedMinutes: null, needsReview: true,
    ...
    },
    {
      ...
      punchCount: 2, workedMinutes: 300, needsReview: false,
  ...
  assert.deepEqual(sheet.getRow(1).values.slice(1), ['ID.', 'Nombre', 'Depart.', 'Tiempo', 'WH', 'Working Days'])

  const reviewRow = sheet.getRow(6)
  assert.equal(reviewRow.getCell(5).value, 'Needs Review')
  assert.equal(reviewRow.getCell(1).fill.fgColor.argb, 'FFFFC7CE')
  assert.equal(sheet.getRow(9).getCell(5).value, 9.83)
